...
import { Round20Rewards } from "./config";

async function main() {
  for (const item of Round20Rewards) {
  ...
  const estimate = BigNumber.from(
    await ethers.provider.call({
      ...
      data: cvxLocker.interface.encodeFunctionData("harvestVotium", [Round20Rewards, 0]),
    })
  );
  ...
  const tx = await cvxLocker.harvestVotium(Round20Rewards, estimate.mul(9).div(10), {

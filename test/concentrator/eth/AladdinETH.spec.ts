...
describe("AladdinETH.spec", async () => {
  ...
  const run = async (
    ...
    context(`${name} in Convex Curve`, async () => {
      beforeEach(async () => {
      ...
      context("deposit/mint/withdraw/redeem", async () => {
        ...
        context("deposit", async () => {
          ...
          it("should succeed", async () => {
            ...
            expect(await rewarder.balanceOf(strategy.address)).to.eq(assetsAmount);
        ...
        context("mint", async () => {
          ...
          it("should succeed", async () => {
            ...
            expect(await rewarder.balanceOf(strategy.address)).to.eq(sharesAmount);
            expect(await aeth.balanceOf(deployer.address)).to.eq(previewAmount);
        ...
        context("withdraw", async () => {
          ...
          it("should succeed", async () => {
            ...
            expect(await rewarder.balanceOf(strategy.address)).to.eq(assetsAmount.sub(withdrawAmount));
        ...
        context("redeem", async () => {
          ...
          it("should succeed", async () => {
            ...
            expect(await rewarder.balanceOf(strategy.address)).to.eq(assetsAmount.sub(redeemAmount.sub(fee)));
      ...
      context("migrate", async () => {
        ...
        it("should revert, when non-owner call", async () => {
          await expect(aeth.connect(signer).migrateStrategy(constants.AddressZero)).to.revertedWith(
            "Ownable: caller is not the owner"
          );
        });

        it("should revert, when new address is zero", async () => {
          await expect(aeth.connect(deployer).migrateStrategy(constants.AddressZero)).to.revertedWith(
            "AladdinCompounder: zero new strategy address"
          );
        });
        ...
        it("should succeed", async () => {
          ...
          const newStrategy = await ConvexCurveAutoCompoundingStrategy.deploy();
          ...
          const token = await ethers.getContractAt("IERC20", config.token, signer);
          await token.approve(aeth.address, constants.MaxUint256);

          // deposit some
          await aeth.connect(signer).deposit(assetsAmount, deployer.address);
          expect(await rewarder.balanceOf(strategy.address)).to.eq(assetsAmount);

          // migrate
          await aeth.connect(deployer).migrateStrategy(newStrategy.address);
          expect(await rewarder.balanceOf(strategy.address)).to.eq(constants.Zero);
          expect(await rewarder.balanceOf(newStrategy.address)).to.eq(assetsAmount);
        });
      });
    });
  };

  const run_frax = async (
    name: string,
    config: {
      fork: number;
      deployer: string;
    ...
    context(`${name} in Convex Frax`, async () => {
      beforeEach(async () => {
        request_fork(config.fork, [config.deployer, config.holder]);
        deployer = await ethers.getSigner(config.deployer);
        signer = await ethers.getSigner(config.holder);
      ...
      context("deposit/mint/withdraw/redeem", async () => {
        ...
        context("distribute harvested reward intermediately", async () => {
          ...
          it("A deposit => harvest => B deposit => A redeem", async () => {
            ...
            expect((await strategy.getUserLocks(deployer.address))[0].balance).to.eq(callRedeemAmount);
            expect(await aeth.totalSupply()).to.eq(totalShares.sub(deployerShare));
            expect(await aeth.totalAssets()).to.eq(totalAssets.sub(callRedeemAmount));
            expect(await aeth.balanceOf(deployer.address)).to.eq(constants.Zero);
          });
        ...
        context("distribute harvested reward in 7 days", async () => {
          ...
          it("A deposit => harvest => B deposit => A redeem => B redeem", async () => {
            ...
            const tx = await aeth.harvest(deployer.address, 0);
            timestamp = (await ethers.provider.getBlock("latest")).timestamp;
            let harvested = (await farm.lockedLiquidityOf(await strategy.vault())).sub(totalAssets);
            expect(harvested).gt(constants.Zero);
            await expect(tx)
              .to.emit(aeth, "Harvest")
              .withArgs(
      ...
      context("migrate", async () => {
        ...
        it("should succeed", async () => {
          ...
          // deposit some
          await aeth.connect(signer).deposit(assetsAmount, deployer.address);
          expect(await farm.lockedLiquidityOf(await strategy.vault())).to.eq(assetsAmount);
          const timestamp = (await ethers.provider.getBlock("latest")).timestamp;
          ...
          // migrate
          await aeth.connect(deployer).migrateStrategy(newStrategy.address);
          expect(await farm.lockedLiquidityOf(await strategy.vault())).to.eq(constants.Zero);
          expect(await farm.lockedLiquidityOf(await newStrategy.vault())).to.eq(assetsAmount);
        });
      });
    });
  };
  ...
  for (const [name, config] of Object.entries(FRAX_UNDERLYING)) {
    run_frax(name, config);
  }
});

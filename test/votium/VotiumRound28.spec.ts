...
const FORK_BLOCK_NUMBER = 15672725;
...
const CRV_HOLDER = "0x5a52e96bacdabb82fd05763e25335261b270efcb";
const UPDATE_ZAP_IMPL = false;

let firstCall = true;
const Round28Rewards = RoundClaimParams[28];

describe("VotiumRound28.spec", async () => {
  ...
  beforeEach(async () => {
    request_fork(FORK_BLOCK_NUMBER, [DEPLOYER, ZAP_OWNER, PROXY_OWNER, CRV_HOLDER, KEEPER]);
  ...
  it("CRV => CVX", async () => {
    ...
    const token = await ethers.getContractAt("IERC20", CRV, signer);
  ...
  it("should harvest", async () => {
    ...
    const amount = await locker.connect(keeper).callStatic.harvestVotium(Round28Rewards, 0);
    await locker.connect(keeper).harvestVotium(Round28Rewards, 0);

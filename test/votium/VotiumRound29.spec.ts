...
const OGN = ADDRESS.OGN;
const OGN_HOLDER = "0x28c6c06298d514db089934071355e5743bf21d60";
const UPDATE_ZAP_IMPL = false;

let firstCall = true;

describe("VotiumRound29.spec", async () => {
  ...
  beforeEach(async () => {
    request_fork(FORK_BLOCK_NUMBER, [DEPLOYER, ZAP_OWNER, PROXY_OWNER, OGN_HOLDER, KEEPER]);
  ...
  it("CRV => CVX", async () => {
    ...
    const signer = await ethers.getSigner(OGN_HOLDER);
    await deployer.sendTransaction({ to: signer.address, value: ethers.utils.parseEther("10") });

    const token = await ethers.getContractAt("IERC20", OGN, signer);

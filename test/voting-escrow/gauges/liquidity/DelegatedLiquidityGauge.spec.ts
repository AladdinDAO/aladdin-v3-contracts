...
describe("DelegatedLiquidityGauge.spec", async () => {
  const Week = 86400 * 7;
  ...
  let gauge: DelegatedLiquidityGauge;
  ...
  beforeEach(async () => {
    ...
    const DelegatedLiquidityGauge = await ethers.getContractFactory("DelegatedLiquidityGauge", deployer);
    ...
    gauge = await DelegatedLiquidityGauge.deploy(minter.getAddress(), proxy.getAddress());
          ...
          beforeEach(async () => {
            if (hasVe) {
              ...
              await boost.boost(
                holder0.address,
                ethers.parseEther("1"),
                Math.floor(timestamp / Week) * Week + Week * 52
              );
              await boost.boost(
                holder1.address,
                ethers.parseEther("1"),

...
import { MockERC20, MockLiquidityManagerImmutable } from "@/types/index";
...
describe("LiquidityManagerBase.spec", async () => {
  ...
  beforeEach(async () => {
    ...
    const MockLiquidityManagerImmutable = await ethers.getContractFactory("MockLiquidityManagerImmutable", deployer);

    token = await MockERC20.deploy("X", "X", 18);
    manager = await MockLiquidityManagerImmutable.deploy(operator.address, token.getAddress());
  });

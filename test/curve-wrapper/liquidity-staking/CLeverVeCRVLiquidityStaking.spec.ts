...
describe("CLeverVeCRVLiquidityStaking.spec", async () => {
  let deployer: SignerWithAddress;
  let operator: SignerWithAddress;

  let cvecrv: CLeverVeCRV;
  let proxy: CurveLockerProxy;
  let staker: CLeverVeCRVLiquidityStaking;
  ...
  beforeEach(async () => {
    ...
    const CLeverVeCRVLiquidityStaking = await ethers.getContractFactory("CLeverVeCRVLiquidityStaking", deployer);
    staker = await CLeverVeCRVLiquidityStaking.deploy(proxy.address, cvecrv.address);
    ...
    // mint aldveCRV
    await cvecrv.mint(deployer.address, ethers.utils.parseEther("1000000"));
  });

  context("auth", async () => {
    it("should initialized correctly", async () => {
      expect(await staker.booster()).to.eq(deployer.address);
      expect(await staker.symbol()).to.eq("stk-cveCRV");
      expect(await staker.name()).to.eq("Staked CLever veCRV");
      expect(await staker.proxy()).to.eq(proxy.address);
      expect(await staker.stakingToken()).to.eq(cvecrv.address);
  ...
  context("deposit", async () => {
    ...
    it("should succeed, when deposit some to self", async () => {
      ...
      await cvecrv.approve(staker.address, amount);
    ...
    it("should succeed, when deposit all to self", async () => {
      const amount = await cvecrv.balanceOf(deployer.address);
      await cvecrv.approve(staker.address, amount);
  ...
  context("withdraw without claim", async () => {
    ...
    beforeEach(async () => {
      await cvecrv.approve(staker.address, amountIn);
    ...
    it("should succeed, when withdraw some to self", async () => {
      const amount = ethers.utils.parseEther("100");
      const before = await cvecrv.balanceOf(deployer.address);
      await expect(staker.withdraw(amount, deployer.address, false))
        .to.emit(staker, "Withdraw")
        .withArgs(deployer.address, deployer.address, amount);
      const after = await cvecrv.balanceOf(deployer.address);
    ...
    it("should succeed, when withdraw some to other", async () => {
      const amount = ethers.utils.parseEther("100");
      const before = await cvecrv.balanceOf(operator.address);
      await expect(staker.withdraw(amount, operator.address, false))
        .to.emit(staker, "Withdraw")
        .withArgs(deployer.address, operator.address, amount);
      const after = await cvecrv.balanceOf(operator.address);
      expect(await staker.balanceOf(deployer.address)).to.eq(amountIn.sub(amount));
      expect(await staker.totalSupply()).to.eq(amountIn.sub(amount));
      expect(after.sub(before)).to.eq(amount);
    });

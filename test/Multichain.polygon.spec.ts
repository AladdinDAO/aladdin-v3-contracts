...
import { IERC20, Layer2CRVDepositor, PolygonCRVDepositor } from "../typechain";
...
const FORK_BLOCK_NUMBER = 27564913;
const CRV = "0x172370d5Cd63279eFa6d502DAB29171933a610AF";
const CRV_HOLDER = "0x06959153B974D0D5fDfd87D561db6d8d4FA0bb0B";
const DEPLOYER = "0x72A53cDBBcc1b9efa39c834A540550e23463AAcB";
const aCRV = "0x89C90E480A39fBE3886bB5bD53ba5B1Acc69D4Fb";
const aCRV_HOLDER = "0x5F160f2EBb42D83426Bcd4092B884ddC7fE97428";
...
const ANYSWAP_ROUTER_V4 = "0x6fF0609046A38D76Bd40C5863b4D1a2dCe687f73";
...
describe("Multichain.ploygon.spec", async () => {
  ...
  beforeEach(async () => {
    request_fork(FORK_BLOCK_NUMBER, [DEPLOYER, aCRV_HOLDER, CRV_HOLDER, ANYCALL_MPC, ANYSWAP_ROUTER_V4_MPC], "polygon");
  ...
  context("Layer2CRVDepositor", async () => {
    ...
    context("abort redeem", async () => {
      beforeEach(async () => {
        await depositor.updateWhitelist([deployer.address], true);
        await acrv.connect(signerACRV).approve(depositor.address, constants.MaxUint256);
        await expect(depositor.connect(signerACRV).redeem(ethers.utils.parseEther("100")))
          .to.emit(depositor, "Redeem")
          .withArgs(signerACRV.address, 0, ethers.utils.parseEther("100"));
        expect(await depositor.redeemOperation()).to.deep.eq([ethers.utils.parseEther("100"), constants.Zero, 0]);
        expect(await depositor.abortable(signerACRV.address)).to.deep.eq([
          ethers.utils.parseEther("100"),
          constants.Zero,
        ]);
      ...
      it("should revert, when amount insufficient", async () => {
        await expect(depositor.connect(signerACRV).abortRedeem(ethers.utils.parseEther("100").add(1))).to.revertedWith(
          "Layer2CRVDepositor: insufficient amount to abort"
        );
      });
      ...
      it("should succeed, when abort 40%", async () => {
        ...
        await expect(depositor.connect(signerACRV).abortRedeem(ethers.utils.parseEther("40")))
          .to.emit(depositor, "AbortRedeem")
          .withArgs(signerACRV.address, 0, ethers.utils.parseEther("40"));
        const balanceAfter = await acrv.balanceOf(signerACRV.address);
        expect(balanceAfter.sub(balanceBefore)).to.eq(ethers.utils.parseEther("40"));
        expect(await depositor.redeemOperation()).to.deep.eq([ethers.utils.parseEther("60"), constants.Zero, 0]);
        expect(await depositor.abortable(signerACRV.address)).to.deep.eq([
          ethers.utils.parseEther("60"),
          constants.Zero,
        ]);
      ...
      it("should succeed, when abort 100%", async () => {
        const balanceBefore = await acrv.balanceOf(signerACRV.address);
        await expect(depositor.connect(signerACRV).abortRedeem(ethers.utils.parseEther("100")))
          .to.emit(depositor, "AbortRedeem")
          .withArgs(signerACRV.address, 0, ethers.utils.parseEther("100"));
        const balanceAfter = await acrv.balanceOf(signerACRV.address);
        expect(balanceAfter.sub(balanceBefore)).to.eq(ethers.utils.parseEther("100"));
      ...
      it("should succeed, when abort pending with ongoing exists", async () => {
        ...
        expect(await depositor.redeemOperation()).to.deep.eq([constants.Zero, ethers.utils.parseEther("100"), 0]);
        expect(await depositor.abortable(signerACRV.address)).to.deep.eq([constants.Zero, constants.Zero]);

        await expect(depositor.connect(signerACRV).redeem(ethers.utils.parseEther("200")))
          .to.emit(depositor, "Redeem")
          .withArgs(signerACRV.address, 1, ethers.utils.parseEther("200"));
        expect(await depositor.redeemOperation()).to.deep.eq([
          ethers.utils.parseEther("200"),
          ethers.utils.parseEther("100"),
          0,
        ]);
        ...
        const balanceBefore = await acrv.balanceOf(signerACRV.address);
        await expect(depositor.connect(signerACRV).abortRedeem(ethers.utils.parseEther("150")))
          .to.emit(depositor, "AbortRedeem")
          .withArgs(signerACRV.address, 1, ethers.utils.parseEther("150"));
        const balanceAfter = await acrv.balanceOf(signerACRV.address);
        expect(balanceAfter.sub(balanceBefore)).to.eq(ethers.utils.parseEther("150"));
        expect(await depositor.redeemOperation()).to.deep.eq([
          ethers.utils.parseEther("50"),
    ...
    context("cross chain redeem", async () => {
      ...
      it("should revert, when no pending async redeem exists", async () => {
        await depositor.connect(signerACRV).abortRedeem(ethers.utils.parseEther("100"));
            ...
            "0x3eb0e35b" +
              defaultAbiCoder
                .encode(
                  ...
                  [0, 1, depositor.address, ethers.utils.parseEther("100"), 0, depositor.address]
                )
                .slice(2),
            depositor.address,
            1
      ...
      it("should succeed, when finalizeRedeem", async () => {
        ...
        expect(await depositor.finialzedRedeemState(0)).to.deep.eq([
          ethers.utils.parseEther("100"),
  ...
  context("PolygonCRVDepositor", async () => {
    let alice: SignerWithAddress;
    let depositor: PolygonCRVDepositor;

    beforeEach(async () => {
      [alice] = await ethers.getSigners();

      const PolygonCRVDepositor = await ethers.getContractFactory("PolygonCRVDepositor", deployer);
      depositor = await PolygonCRVDepositor.deploy();
      await depositor.initialize(
        anycall.address,
        router.address,
        crossChainCallProxy.address,
    ...
    context("cross chain deposit", async () => {
      ...
      it("should revert, when insufficient cross chain amount", async () => {
        ...
        await expect(depositor.prepareAsyncDeposit()).to.revertedWith(
          "PolygonCRVDepositor: insufficient cross chain amount"
        );
      });
      ...
      it("should succeed, when prepareAsyncDeposit", async () => {
        ...
        await expect(depositor.prepareAsyncDeposit())
          ...
          .withArgs(0, ethers.utils.parseEther("100"), ethers.utils.parseEther("0.1"), ethers.utils.parseEther("0"));
        expect(await depositor.depositOperation()).to.deep.eq([constants.Zero, ethers.utils.parseEther("100"), 0]);
        expect(await depositor.abortable(signerCRV.address)).to.deep.eq([constants.Zero, constants.Zero]);
      ...
      it("should succeed, when finalizeDeposit", async () => {
        ...
        expect(await depositor.claimable(signerCRV.address)).to.deep.eq([
          ethers.utils.parseEther("149"),
          constants.Zero,
        ]);

        await acrv.connect(signerACRV).transfer(depositor.address, ethers.utils.parseEther("149"));
        const balanceBefore = await acrv.balanceOf(signerCRV.address);
        await expect(depositor.connect(signerCRV).claim())
          .to.emit(depositor, "Claim")
          .withArgs(signerCRV.address, ethers.utils.parseEther("149"), constants.Zero);
    ...
    context("async exit", async () => {
      ...
      it("should revert, when non-whitelist call asyncExit", async () => {
        await expect(depositor.connect(alice).asyncExit("0x")).to.revertedWith("Layer2CRVDepositor: only whitelist");
      });

      it("should revert, when no pending deposit", async () => {
        await expect(depositor.asyncExit("0x")).to.revertedWith("PolygonCRVDepositor: no pending deposit");
      });

      it("should succeed, when call asyncExit", async () => {
        await depositor.prepareAsyncDeposit();
        await expect(depositor.asyncExit("0x000000"))
          .to.emit(depositor, "AsyncExit")
          .withArgs(0)
          .to.emit(anycall, "LogAnyCall")
          .withArgs(
            crossChainCallProxy.address,
            depositor.address,
            "0x1029c1da" + defaultAbiCoder.encode(["bytes"], ["0x000000"]).slice(2),
            constants.AddressZero,
            1
          );
      });

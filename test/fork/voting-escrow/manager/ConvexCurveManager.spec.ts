...
import { ConvexCurveManager, IConvexBasicRewards, LiquidityGauge, MockERC20 } from "@/types/index";
...
describe("ConvexCurveManager.spec", async () => {
  ...
  let manager: ConvexCurveManager;
  ...
  for (const testcase of TestCases) {
    context(`test with token[${testcase.name}] in rewarder[${testcase.rewarder}]`, async () => {
      beforeEach(async () => {
        ...
        const ConvexCurveManager = await ethers.getContractFactory("ConvexCurveManager", deployer);
        ...
        manager = await ConvexCurveManager.deploy();

        await gauge.initialize(token.getAddress());
        await manager.initialize(gauge.getAddress(), token.getAddress(), rewarder.getAddress());
      ...
      context("constructor", async () => {
        ...
        it("should revert, when initialize again", async () => {
          await expect(manager.initialize(ZeroAddress, ZeroAddress, ZeroAddress)).to.revertedWith(

...
describe("MarketV2.spec", async () => {
  ...
  context("mint and redeem", async () => {
    beforeEach(async () => {
      ...
      await expect(treasury.connect(signer).initializeProtocol(ethers.parseEther("101")))
    ...
    context("normal mode", async () => {
      context("mintFToken", async () => {
        ...
        it("should succeed", async () => {
          ...
          expect(await treasury.totalBaseToken()).to.eq(ethers.parseEther("102.007475"));
        });
      });
      ...
      context("mintXToken", async () => {
        ...
        it("should succeed", async () => {
          ...
          expect(await treasury.totalBaseToken()).to.eq(ethers.parseEther("101.9999"));
        });
      });
      ...
      context("redeemFToken", async () => {
        ...
        it("should succeed", async () => {
          ...
          expect(await treasury.totalBaseToken()).to.eq(ethers.parseEther("100.9995"));
        });
      });

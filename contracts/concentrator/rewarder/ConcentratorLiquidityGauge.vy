...
def __init__(_lp_token: address, _minter: address, _admin: address):
    ...
    self.inflation_rate = CTR20(cont_token).rate()
    self.future_epoch_time = CTR20(cont_token).future_epoch_time_write()
...
def _update_liquidity_limit(addr: address, l: uint256, L: uint256):
    """
    @notice Calculate limits which depend on the amount of CTR token per-user.
            Effectively it calculates working balances to apply amplification
            of CTR production by CTR
...
def _checkpoint(addr: address):
    ...
    if prev_future_epoch >= _period_time:
        ...
        self.future_epoch_time = CTR20(_token).future_epoch_time_write()
        new_rate = CTR20(_token).rate()
...
def reward_contract() -> address:
    """
    @notice Address of the reward contract providing non-CTR incentives for this gauge
...
def set_killed(_is_killed: bool):
    ...
    @dev When killed, the gauge always yields a rate of 0 and so cannot mint CTR

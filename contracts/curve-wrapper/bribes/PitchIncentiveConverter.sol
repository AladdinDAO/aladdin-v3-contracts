...
import "./interfaces/IPitchIncentivesStash.sol";
import "../interfaces/ICurveLockerProxy.sol";
import "../../interfaces/IZap.sol";

import "./BaseBribeConverter.sol";

contract PitchIncentiveConverter is BaseBribeConverter {
  ...
  function convert(IPitchIncentivesStash.ClaimData[] calldata _claims, uint256 _minOut)
    external
    onlyKeeper
    returns (uint256 _amountCRV)
  {
    ...
    for (uint256 i = 0; i < _length; i++) {
      IPitchIncentivesStash.ClaimData memory _claim = _claims[i];
      if (!IPitchIncentivesStash(_stash).isClaimed(_claim.token, _claim.index)) {
        IPitchIncentivesStash(_stash).claim(_claim.token, _claim.index, proxy, _claim.amount, _claim.merkleProof);
      }

      // use local bitmap to avoid request mutiple rewards from proxy.
      uint256 _claimPeriod = IPitchIncentivesStash(_stash).claimPeriod(_claim.token);
